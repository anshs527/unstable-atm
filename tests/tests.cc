...
// Test case for depositing negative amount
TEST_CASE("Example: Simple deposit negative amount", "[ex-7]") {
  Atm atm;
  atm.RegisterAccount(12345678, 1234, "Sam Sepiol", 300.30);
  atm.DepositCash(12345678, 1234, 20.0);
  auto accounts = atm.GetAccounts();
  Account sam_account = accounts[{12345678, 1234}];

  REQUIRE(sam_account.balance == 320.30);
  REQUIRE_THROWS_AS(atm.DepositCash(12345678, 1234, -20.0),
                    std::invalid_argument);
}